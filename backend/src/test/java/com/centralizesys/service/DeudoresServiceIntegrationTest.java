...
class DeudoresServiceIntegrationTest extends BaseIntegrationTest {
    ...
    void shouldHandleRounding() {
        ...
        Long userId = createTestUser();
        Venta venta = new Venta(null, LocalDateTime.of(2026, java.time.Month.JANUARY, 1, 12, 0), "Math User", 10.0, 0.0, "MINORISTA", userId, "ACTIVA", 0.0);
        Long ventaId = ventaRepository.saveVenta(venta);
    ...
    void shouldRecordPaymentInHistory() {
        Long userId = createTestUser();
        Venta venta = new Venta(null, LocalDateTime.of(2026, java.time.Month.JANUARY, 1, 12, 0), "History User", 100.0, 0.0, "MINORISTA", userId, "ACTIVA", 0.0);
        Long ventaId = ventaRepository.saveVenta(venta);
